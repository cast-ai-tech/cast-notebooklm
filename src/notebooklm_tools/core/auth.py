...
from notebooklm_tools.core import crypto
...
def load_cached_tokens() -> AuthTokens | None:
    ...
    try:
        ...
        return tokens
    except (json.JSONDecodeError, KeyError, TypeError, crypto.EncryptionKeyError) as e:
...
def save_tokens_to_cache(tokens: AuthTokens, silent: bool = False) -> None:
    ...
    try:
        ...
        raise
    with f:
        json.dump(_encrypt_json(tokens.to_dict()), f, indent=2)
...
class AuthManager:
    ...
    def load_profile(self, force_reload: bool = False) -> Profile:
        ...
        try:
            cookies_envelope = json.loads(self.cookies_file.read_text(encoding="utf-8"))
            cookies = _decrypt_json(cookies_envelope)
            metadata = {}
            if self.metadata_file.exists():
                metadata_envelope = json.loads(self.metadata_file.read_text(encoding="utf-8"))
                metadata = _decrypt_json(metadata_envelope)
    ...
    def save_profile(
        ...
        if not force and email and self.metadata_file.exists():
            try:
                existing_envelope = json.loads(self.metadata_file.read_text(encoding="utf-8"))
                existing_metadata = _decrypt_json(existing_envelope)
            ...
            except (json.JSONDecodeError, KeyError, crypto.EncryptionKeyError):
        ...
        try:
            ...
            raise
        with f:
            json.dump(_encrypt_json(cookies), f, indent=2, ensure_ascii=False)
        ...
        try:
            ...
            raise
        with f:
            json.dump(_encrypt_json(metadata), f, indent=2, ensure_ascii=False)
