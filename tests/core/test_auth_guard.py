...
from notebooklm_tools.core.auth import AuthManager, _encrypt_json  # noqa: E402
...
class TestSaveProfileMismatchGuard:
    ...
    def _create_existing_profile(self, tmp_path: Path, email: str) -> AuthManager:
        """Helper: create a profile with existing (encrypted) credentials on disk.

        Credentials are always encrypted at rest (see core/crypto.py), so the
        fixture writes the same envelope format AuthManager.save_profile does.
        """
        profiles_dir = tmp_path / "profiles" / "test-profile"
        profiles_dir.mkdir(parents=True)

        cookies = [{"name": "SID", "value": "old-sid"}]
        (profiles_dir / "cookies.json").write_text(json.dumps(_encrypt_json(cookies)))
...
class TestBuildLabelExtraction:
    ...
    def test_profile_build_label_defaults_none_for_old_profiles(self, tmp_path, monkeypatch):
        ...
        from notebooklm_tools.core.auth import AuthManager, _encrypt_json
        ...
        # Simulate an old profile without build_label (but still encrypted --
        # credentials are always encrypted at rest, see core/crypto.py)
        profile_dir = tmp_path / "old-profile"
        profile_dir.mkdir(parents=True)
        (profile_dir / "cookies.json").write_text(json.dumps(_encrypt_json({"SID": "abc"})))
